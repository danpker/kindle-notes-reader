...
impl Note {
    ...
    fn from_raw_string(raw_note: &str) -> Option<Note> {
...
/// for each note.
pub fn create_notes_from_string(clippings_text: &str) -> Vec<Note> {
    let mut output = Vec::new();
    for line in clippings_text.split("==========") {
        if let Some(note) = Note::from_raw_string(line) {
            output.push(note);
        }
    }
    output
...
mod tests {
    ...
    fn test_sets_raw_note_to_raw_input() {
        ...
        let note = Note::from_raw_string(raw_note).unwrap();
